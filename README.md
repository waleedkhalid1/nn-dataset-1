...
3. The constructor of this <strong>Net</strong> class has following parameters: <strong> in_shape: tuple, out_shape: tuple, prms: dict </strong>
   - <strong>in_shape</strong> is extracted from the first tensor of dataset iterator and describes a shape of this tensor, e.g., for images (batch, channel, width, hight)
...
6. Every class <strong>Net</strong> implements two functions: `train_setup(self, device, prms)` and `learn(self, train_data)`. The first function initializes the `criteria` and `optimizer`, while the second implements the training pipeline. See a simple implementation in the https://github.com/ABrain-One/nn-dataset/blob/main/ab/nn/dataset/AlexNet.py   
