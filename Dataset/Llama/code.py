...
class DatasetPreparation(Dataset):
    def __init__(self, text_data: str, seq_length: int = 128) -> None:
    ...
    def __getitem__(self, index) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        X = torch.tensor(self.X[start_idx:end_idx], dtype=torch.long).to(device)
        y = torch.tensor(self.X[start_idx + 1:end_idx + 1], dtype=torch.long).to(device)
        return X, y

    def string_to_vector(self, name: str) -> list[int]:
        return [self.char_to_idx[s] for s in name]
