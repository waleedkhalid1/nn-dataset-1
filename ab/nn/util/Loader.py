...
class Loader:
    ...
    def load_dataset(loader_path, transform_path=None, **kwargs):
        ...
        if transform_path:
            ...
            transform = getattr(__import__(nn_mod(transform_module), fromlist=[transform_func]), transform_func)()

        # Dynamically load the loader function
        loader_module, loader_func = loader_path.rsplit('.', 1)
        loader = getattr(__import__(nn_mod(loader_module), fromlist=[loader_func]), loader_func)
