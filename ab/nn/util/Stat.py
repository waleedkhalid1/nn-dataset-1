...
def unique_configs(config_pattern):
    # Ensure patterns is a string
    patterns = config_pattern
    if isinstance(patterns, tuple):
        patterns = "_".join(map(str, patterns))  # Convert tuple to string with underscores

    try:
        directory_path = join(Const.stat_dir_global, patterns)
        makedirs(directory_path, exist_ok=True)
        return [patterns]  # Example of returning the processed string
