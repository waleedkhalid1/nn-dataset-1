...
# Function to return supported hyperparameters
def supported_hyperparameters():
    return {'lr', 'momentum', 'dropout'}

# DarkNetUnit: A basic building block of the network
...
class Net(nn.Module):
    def __init__(self, 
        ...
        learning_rate = prms.get('lr', 0.01)
        momentum = prms.get('momentum', 0.9)
        dropout = prms.get('dropout', 0.5)
        ...
        # Compute final feature map size after pooling
    ...
    def train_setup(self, device: torch.device, prms: dict):
        learning_rate = float(prms.get("lr", 0.01))
        momentum = float(prms.get("momentum", 0.9))
